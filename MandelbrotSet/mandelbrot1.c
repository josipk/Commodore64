...
**************************************/
//Build: cl65 mandelbrot1.c runtime.lib -o mandelbrot1.prg
...
void generateMandlebrotSet(void) {
	...
	for(py=0; py<rows; ++py) {
		...
		for(px=0; px<columns; ++px) {
			...
			//POKE(screen_loc+u, 160);
			POKE(screen_loc+u, 1+c);
			POKE(color_loc+u, c);
...
int main(void) {
	...
	aa=fdiv(itof(35),itof(10)); //3.5
	bb=fdiv(itof(25),itof(10)); //2.5
	cc=itof(2); //2.0
	dd=itof(1); //1.0
	ff=itof(4);  //4.0
