/************************************** 
* CC65 Tiny-Pong in C v.0.2           *
* josip.kalebic@gmail.com, 2022.      *
**************************************/
//Build: cl65 tiny_pong2.c -o tiny_pong2.prg
...
void setupSprite(void) {
	...
	POKE(SPR + 21, 15); //enable sprite 1 and 2
	POKE(0x07F8, 200);  //0x07F8 = 2040 
	POKE(0x07F9, 200);  //0x07F8 = 2040 
	POKE(0x07FA, 200);  //0x07F8 = 2040 	
	POKE(0x07FB, 200);  //0x07F8 = 2040 
...
void checkKeyPress(void) {
	...
	if (k1==62 && keypause ==0) {
		...
		if(PEEK(SPR + 29) == 0) {
			...
			POKE(SPR + 29, 15); // sprite double width no
			POKE(SPR + 23, 15); // sprite double height no		
